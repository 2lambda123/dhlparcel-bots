...
class Jobqueue(object):
    ...
    def addjob(self, task, priority):
        ...
        for job in self.jobqueue:
            if job[TASK] == task:
                if job[PRIORITY] != priority:  # change priority. is this useful?
                    ...
                    self.logger.info(u'Duplicate job, changed priority to %(priority)s: %(task)s',
        ...
        self.logger.info(u'Added job %(job)s, priority %(priority)s: %(task)s', {
...
def start():
    ...
    logger.log(25, u'Bots %(process_name)s listens for xmlrpc at port: "%(port)s".',
